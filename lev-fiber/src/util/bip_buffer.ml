...
module Make (Array : Array) = struct
  ...
  let _invariant t =
    if t.b_inuse then
      assert (t.b_end - t.a_start > Array.length t.buf - t.a_end)
  ...
  let unused t =
    if t.b_inuse then t.a_start - t.b_end else Array.length t.buf - t.a_end

  let write_start t = if t.b_inuse then t.b_end else t.a_end

  let set_current_buffer t =
    let space_left_for_a = Array.length t.buf - t.a_end in
